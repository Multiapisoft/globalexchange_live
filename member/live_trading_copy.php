    ...
    async function fetchBybitData() {
        try {
            ...
            const [priceResponse, orderBookResponse, tradesResponse] = await Promise.all([
                ...
                fetch('https://api.bybit.com/v5/market/orderbook?category=linear&symbol=BTCUSDT&limit=10', {
                    signal: controller.signal
                }),
                fetch('https://api.bybit.com/v5/market/recent-trade?category=linear&symbol=BTCUSDT&limit=10', {
                    signal: controller.signal
                })
            ]);
