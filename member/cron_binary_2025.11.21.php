...
while ($row = mysqli_fetch_object($result)) {
    ...
    /*elseif($min_three<3 && $amount_left != 0 && $amount_right != 0){
        ...
        if ($amount_left > $amount_right) {
            $matching_amount = $amount_right >= 500000 ? 500000 : $amount_right;
            $left_carry = $amount_left - $amount_right;
            $right_carry = 0;
        } elseif ($amount_right == $amount_left) {
            $matching_amount = $amount_left >= 500000 ? 500000 : $amount_left;
            $right_carry = 0;
            $left_carry = 0;
        } elseif ($amount_right > $amount_left) {
            $matching_amount = $amount_left >= 500000 ? 500000 : $amount_left;
        ...
        $balance = $matching_amount * $per / 100;
...
function get_amount($id_array)
{
    ...
    if (!empty($id_array)) {
        ...
        $amount = mysqli_fetch_object(mysqli_query($link, "SELECT sum(amount) as amount FROM investments WHERE uid IN ( $uid_in ) AND binary_status=0 "))->amount;
    }
    return $amount;
}
...
mysqli_query($link, "UPDATE income_growth SET binary_status=1 WHERE binary_status=0");
