...
$query = "SELECT m.`recid`, m.`sender`, m.`receiver`, m.`subject`, m.`message`, m.`filename`, m.`datetime`, m.`read`"
        ...
        . " WHERE m.recid='".$recid."' AND (m.sender='$uid' OR m.receiver='$uid')";
                    ...
                    <div class="col-xs-12 col-sm-12 col-md-9 p-0 inbox-mail">
                        <div class="inbox-avatar p-20 border-btm">
                            ...
                            <div class="inbox-avatar-text">
                                <div class="avatar-name"><strong><?php if($row->sender!=$uid){echo "From";}else{echo 'To';}?>: </strong>
                                    <?php /*if($row->sender!=0 && $row->sender!=$uid){echo $row->name;}elseif($row->receiver!=0 && $row->receiver!=$uid){echo $row->r_name;}else{echo 'Company';}echo ' - ';*/?><em><?php if($row->sender!=0 && $row->sender!=$uid){echo $row->login_id;}elseif($row->receiver!=0 && $row->receiver!=$uid){echo $row->r_login_id;}else{echo 'Company';}?></em>
                                </div>
