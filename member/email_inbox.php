...
$query = "SELECT m.`recid`, m.`sender`, m.`receiver`, LEFT(m.subject,50) as subject, LEFT(m.message,50) as message, m.`filename`, m.`datetime`, m.`read`"
...
<style>
/* Inbox — match compose mail dark/gold theme */
...
.ge-inbox-item.unread {
  border-color: rgba(212, 175, 55, 0.28);
  background: rgba(212, 175, 55, 0.06);
}
.ge-inbox-item.unread .ge-inbox-from,
.ge-inbox-item.unread .ge-inbox-subject {
  color: #fff;
  font-weight: 700;
}
...
<div class="ge-mail">
  <div class="ge-mail-head">
    <h1>Inbox</h1>
    <p>Messages from support<?php echo $login_id !== '' ? ' · ' . htmlspecialchars($login_id) : ''; ?></p>
  </div>

  <div class="ge-panel">
    <nav class="ge-mail-tabs" aria-label="Mailbox">
      <a href="email_inbox.php" class="active"><i class="fa fa-inbox"></i> Inbox</a>
  ...
  <div class="ge-panel">
    <div class="ge-mail-meta">
      ...
      <span>
        ...
        <?php if ($unread > 0) { ?>
          · <span class="unread-pill"><?php echo (int) $unread; ?> unread</span>
        <?php } ?>
      </span>
    </div>
    ...
    <?php } else { ?>
      <div class="ge-inbox-list">
        <?php foreach ($rows as $row) {
            $is_unread = ((int) $row->read === 0);
            $from_name = ((int) $row->sender !== 0)
                ? trim($row->name . (!empty($row->login_id) ? ' (' . $row->login_id . ')' : ''))
                : 'Admin';
            $initial = strtoupper(substr($from_name, 0, 1));
            if ($initial === '') {
                $initial = 'A';
            }
            $dt = strtotime($row->datetime);
        ?>
          <a href="email.php?recid=<?php echo (int) $row->recid; ?>" class="ge-inbox-item<?php echo $is_unread ? ' unread' : ''; ?>">
            <div class="ge-inbox-avatar"><?php echo htmlspecialchars($initial); ?></div>
            <div class="ge-inbox-body">
              <div class="ge-inbox-top">
                <div class="ge-inbox-from"><?php echo htmlspecialchars($from_name); ?></div>
