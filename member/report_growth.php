...
$title = ($type == 2) ? "Super Jackpot Income" : (($type == 1) ? "Stacking Referral Income" : "Trubot Trading Income");
...
/* Fresh Growth Theme - Same as Downline */
* {
    margin: 0;
    padding: 0;
...
.fresh-growth-header-content {
    position: relative;
    z-index: 2;
}

.fresh-growth-header h1 {
    font-size: 2.5rem;
    font-weight: 900;
    margin-bottom: 12px;
...
<script>
// Fresh Growth Search Functionality
document.addEventListener('DOMContentLoaded', function() {
    const searchInput = document.getElementById('growthSearchInput');
        ...
        if (visibleRows.length === 0 && searchTerm !== '') {
            ...
            noResultsRow.innerHTML = `
                <td colspan="6" style="text-align: center; padding: 40px; color: var(--text-muted); font-size: 1.1rem;">
                    ...
                    No growth payments found matching "${searchTerm}"
                </td>
            `;
    ...
    const badges = document.querySelectorAll('.fresh-plan-badge, .fresh-percentage-badge, .fresh-days-badge');
