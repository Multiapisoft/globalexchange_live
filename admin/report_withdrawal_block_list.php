...
$column = array("i", "c", "login_id", "datetime", "amount", "fee", "net_amount", "amount_coin", "withdrawal_address", "remark", "topup", "wallet", "wallet_promo", "type", "action");

$select = "d.*, u.login_id, u.topup, u.wallet, u.wallet_promo";

$query = "SELECT __SELECT__ FROM withdrawal_block as d LEFT JOIN user as u ON u.uid=d.uid WHERE d.status = '".$type."'";
$search = "";
...
while ($row = my_fetch_object($result)){$i++;
    $st = "Approved";
    if($row->status == 0){
        $st = '<a href="withdrawal_block_model.php?recid='.$row->recid.'" onclick="return confirm(\'Are you sure you want to approve?\');">Approve</a> | <a href="withdrawal_block_model.php?recid='.$row->recid.'&cancel=1" onclick="return confirm(\'Are you sure you want to cancel?\');">Cancel</a>';
    }
    elseif($row->status == 2){
        $st = "Cancelled";
    }
	...
	$data[] = array(
		$i,
		'<input class="wid" name="recid[]" value="'.$row->recid.'" type="checkbox">',
		$row->login_id,
